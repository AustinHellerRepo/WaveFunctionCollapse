...
impl SudokuPuzzle {
    ...
    fn get_solution(&self) -> Result<SudokuPuzzle, String> {
        ...
        let mut exclusive_node_state_collections: Vec<NodeStateCollection> = Vec::new();
        ...
        let mut possible_node_state_collection_per_number: HashMap<u8, NodeStateCollection> = HashMap::new();
        for number in 1u8..10 {
            let mut node_state_ids: Vec<String> = Vec::new();
            for state in 1u8..10 {
                if state != number {
        ...
        let mut always_node_state_collection_per_to_number_per_from_number: HashMap<u8, HashMap<u8, NodeStateCollection>> = HashMap::new();
        for from_number in 1u8..10 {
            let from_number_node_state_id = format!("state_{from_number}");
            let mut always_node_state_collection_per_to_number: HashMap<u8, NodeStateCollection> = HashMap::new();
        ...
        let mut node_state_collection_per_id: HashMap<String, NodeStateCollection> = HashMap::new();
